...
function createWasmAudioWorkletProcessor(audioParams) {
  class WasmAudioWorkletProcessor extends AudioWorkletProcessor {
    constructor(args) {
      ...
      this.callback = ((a1, a2, a3, a4, a5, a6, a7) => abort('Internal Error! Attempted to invoke wasm function pointer with signature "iiiiiiii", but no such functions have gotten exported!'));
...
function findWasmBinary() {
  return locateFile('emscripten-audioworklet-worker.wasm');
}
...
// Begin JS library code
  ...
  var __emscripten_create_audio_worklet = (wwID, contextHandle, stackLowestAddress, stackSize, callback, userData) => {
      ...
      locateFile('emscripten-audioworklet-worker.js')
  ).then(() => {
  ...
  var __emscripten_create_wasm_worker = (wwID, stackLowestAddress, stackSize, pthreadPtr) => {
      if (!_emscripten_has_threading_support()) {
        err('create_wasm_worker: environment does not support SharedArrayBuffer, wasm workers are not available');
        return false;
      }
      let worker;
      worker = _wasmWorkers[wwID] = new Worker(
      locateFile('emscripten-audioworklet-worker.js')
  , {
  ...
  var _emscripten_audio_worklet_post_function_v = (audioContext, funcPtr) => {
  ...
  var _emscripten_current_thread_is_audio_worklet = () => ENVIRONMENT_IS_AUDIO_WORKLET;
  ...
  function _emscripten_force_exit(status) {
    assert(!ENVIRONMENT_IS_WASM_WORKER, "attempt to call proxied function '_emscripten_force_exit' from a Wasm Worker (where proxying is not possible)");
...
function assignWasmExports(wasmExports) {
  ...
  function assignWasmImports() {
    ...
    /** @export */
    emscripten_audio_worklet_post_function_v: _emscripten_audio_worklet_post_function_v,
    /** @export */
    ...
    /** @export */
    emscripten_terminate_all_wasm_workers: _emscripten_terminate_all_wasm_workers,
    /** @export */
